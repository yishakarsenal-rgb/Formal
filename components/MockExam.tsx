...
import { examQuestions, EXAM_TIME_SECONDS, type ExamQuestion } from '@/lib/exam-data'
...
function SetupScreen({
  ...
  const filtered = selectedChapter === 'All Chapters'
    ? examQuestions
    : examQuestions.filter((q) => q.chapter === selectedChapter)

  const finalCount = selectedCount === 'All'
    ? filtered.length
    ...
    <div className="flex flex-col items-center justify-center min-h-full px-4 py-12">
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-10">
          ...
          <h2 className="text-3xl font-bold text-foreground mb-2">Mock Examination</h2>
          <p className="text-muted-foreground text-sm leading-relaxed">
            Configure your exam below. You will have{' '}
            <span className="font-semibold text-foreground">120 minutes</span> to complete
        ...
        <div className="bg-card border border-border rounded-2xl p-6 shadow-sm flex flex-col gap-6">
          ...
          <div>
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              {filtered.length} question{filtered.length !== 1 ? 's' : ''} available in this selection
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 p-3 rounded-xl bg-muted/60 border border-border">
            ...
            <span className="text-lg font-bold text-foreground font-mono">120:00</span>
          </div>
          ...
          >
            ...
            Begin Examination ({finalCount} question{finalCount !== 1 ? 's' : ''})
          </Button>
        </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-3">
          {[
            { label: 'Total Questions', value: examQuestions.length, icon: BookOpen },
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-border bg-card/80 backdrop-blur shrink-0">
        <div className={cn(
          ...
          timerWarning ? 'bg-destructive/10 text-destructive' : 'bg-muted text-foreground'
        )}>
...
function ResultsScreen({
  ...
  const gradeColor =
    grade === 'A' ? 'text-emerald-600 dark:text-emerald-400' :
  ...
  const byChapter = questions.reduce<Record<string, { correct: number; total: number }>>((acc, q, i) => {
      ...
      <div className="max-w-2xl mx-auto w-full px-4 py-6 flex flex-col gap-6">
        ...
        <div className="bg-card border border-border rounded-2xl p-6 text-center shadow-sm">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h2 className="text-xl font-bold text-foreground">Examination Results</h2>
          </div>
          <div className={cn('text-6xl font-black mb-2', gradeColor)}>{grade}</div>
        ...
        <div className="bg-card border border-border rounded-2xl p-4 shadow-sm">
          ...
          <div className="flex flex-col gap-2">
            {Object.entries(byChapter).map(([chapter, { correct: c, total: t }]) => (
              <div key={chapter} className="flex items-center gap-3">
                <span className="text-xs text-muted-foreground w-20 shrink-0">{chapter}</span>
              ...
              return (
                <div key={q.id}>
                  ...
                  >
                    ...
                    <Badge className={cn('text-xs shrink-0', difficultyColor(q.difficulty))}>
                      {q.difficulty}
                    </Badge>
                  </button>
                  {reviewIdx === i && (
                        ...
                        {q.options.map((opt, oi) => (
                          <div
                            ...
                            className={cn(
                              ...
                              oi === q.answer && 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-800 dark:text-emerald-300 font-medium',
                              oi === answers[i] && oi !== q.answer && 'bg-red-50 dark:bg-red-900/20 text-red-800 dark:text-red-300'
                            )}
                          >
                            <span className="shrink-0">
                              {oi === q.answer ? '✓' : oi === answers[i] ? '✗' : '·'}
                            </span>
                            <span>{opt}</span>
                          </div>
                        ))}
...
export function MockExam() {
  ...
  if (phase === 'running') return <RunningExam questions={activeQuestions} onFinish={handleFinish} />
